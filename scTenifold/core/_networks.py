...
def pc_net_calc(
    data: pd.DataFrame,  # genes x cells
    selected_samples,
    n_comp: int = 3,
...
def make_networks(
    ...
    **kwargs,
    ...
    for i, pc_net in enumerate(results):
        ...
        temp_df.loc[sel_genes, sel_genes] = pd.DataFrame(
            pd.DataFrame(pc_net, index=Z.index, columns=Z.index),
...
def manifold_alignment(X: pd.DataFrame, Y: pd.DataFrame, d: int = 30, tol: float = 1e-8, **kwargs) -> pd.DataFrame:
    ...
    W = -np.concatenate(
        (np.concatenate((w_X, w_XY), axis=1), np.concatenate((w_XY.T, w_Y), axis=1)),
        axis=0,
    )
