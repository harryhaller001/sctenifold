...
def calc_U_stat_df(
    features,
    df: pd.DataFrame,
    neg_features: list[str] | None = None,
    max_rank=1500,
    w_neg=1,
):
    """Calculate U stat df."""
...
def cal_Uscore(X: pd.DataFrame, pos_genes, neg_genes, max_rank=1500, w_neg=1, ties_method="average"):
    """Calculate U score."""
    ranked_df = X.rank(ascending=False, method=ties_method)
    pos_genes = _check_features(X, pos_genes)
    cell_auc = calc_U_stat_df(pos_genes, ranked_df, neg_features=neg_genes, max_rank=max_rank, w_neg=w_neg)
