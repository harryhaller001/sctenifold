...
def get_config_file(
    config_type: int = typer.Option(
        1, "--type", "-t", help="Type, 1: scTenifoldNet, 2: scTenifoldKnk", min=1, max=2
    ),
    file_path: str = typer.Option(
        ".config.yml", "--path", "-p", help="Path to generate empty config file"
    ),
):
    config = (
        scTenifoldNet.get_empty_config()
        if config_type == 1
        else scTenifoldKnk.get_empty_config()
    )
...
def build_net(
    config_file_path: str = typer.Option(
        ..., "--config", "-c", help="Loaded config file's path"
    ),
